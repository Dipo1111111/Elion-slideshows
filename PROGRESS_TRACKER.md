...
- [ ] **Pro caps (placeholders, set before launch):** monthly gen cap (currently 100, lowered from 500 on 2026-08-04) + project cap (currently 5). **Model locked 2026-08-05: `big-pickle` on OpenCode Zen (free model, verified live), env-driven via `OPENCODE_MODEL`; OpenRouter retired.**
- [ ] **Pro margin math (before launch, user 2026-08-04):** compute real per-generation cost (OpenCode LLM + Apify scrape amortized across cached backgrounds + storage/bandwidth) and validate the $19/mo / $99-yr / 100-cap pricing leaves margin; lower the cap if not. PRD §6
- [ ] **Google OAuth (user):** Google Cloud Console → OAuth consent screen (External, email+profile) → OAuth client ID (Web app) → Authorized JS origins `https://wadmyrurjnlvbrugcnud.supabase.co` + `http://localhost:5173`, redirect URI `https://wadmyrurjnlvbrugcnud.supabase.co/auth/v1/callback` → paste Client ID + Secret into Supabase → Auth → Providers → Google → Enable. Frontend button already wired (`supabase.auth.signInWithOAuth`).
- [ ] **Lemon Squeezy (user):** store URL + Pro monthly ($19) variant ID + Pro annual ($99) variant ID + webhook secret → set `LEMON_SQUEEZY_*` env; webhook points at `POST /api/lemon/webhook` (route verified wired; 503 until secret set).
