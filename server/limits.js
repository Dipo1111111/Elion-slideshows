...
//   pro:  X/month (placeholder 100), month-windowed
...
export function remainingFor(profile) {
  ...
  return Math.max(0, LIMITS.monthly - used)
}
...
export async function assertCanGenerate(profile) {
  ...
  if (profile.plan === 'pro' && remainingFor(profile) <= 0) {
    throw new HttpError(403, 'You have used all your slideshows this month. Upgrade or wait for the next cycle.')
  }
}

// Increment counters on success only. Monthly window resets for pro users.
export async function chargeGeneration(userId, plan) {
  ...
  const patch = {
    ...
    monthly_gens: plan === 'pro' ? (reset ? 1 : (profile.monthly_gens || 0) + 1) : profile.monthly_gens || 0,
    month_start: plan === 'pro' && reset ? now.toISOString() : profile.month_start,
  }
...
export async function canCreateProject(userId, plan) {
  ...
  return count < LIMITS.projects[plan] || LIMITS.projects[plan] === Infinity
}
